...
1. Connect the services to Chat. Each service (Reminders, Notes, Messages, Mail, Contacts, Phone) has its own tab in Settings → Tools with a Connect button, a Connected chip, and Revoke access. Settings → Apple Services is an overview of every service, including Calendar, with its connection status; selecting a name opens its Tools tab. macOS grants Reminders, Contacts, and Automation separately. Revoke access is an in-app switch that stops every agent from using the service immediately (and cancels in-flight operations); removing the macOS permission itself still happens in System Settings. Notes, Mail, and Messages may launch without taking focus when an operation needs them.
2. Open an agent → Tools. Each service is a separate tool. Enable the tool, then its service grant. Select specific lists, folders, contact accounts, mailboxes, or conversations, or explicitly allow all. New services and scopes start off.
