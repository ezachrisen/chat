...
This design adds a SwiftData **generation-turn** log that is queryable independently of the chat bubbles. Every recorded destination-resolved model invocation — direct reply, per-agent group reply, or heartbeat, including an exact heartbeat pass — writes one `GenerationTurn` plus zero or more `ToolInvocation` rows. The visible transcript stays in `StoredChatMessage`; a heartbeat pass simply has no assistant-message link because it posts no bubble. A per-agent **Debug log** toggle (off by default) gates a separate `GenerationDebugPayload` that stores the prompt, Apple transcript / OpenAI tool-loop messages, reasoning entries, and unsanitized model output. Debug-enabled heartbeat passes retain that full payload and their correlated collaboration trace. `HeartbeatRun` is kept as the compact Heartbeats-window row; bulky prompt/output moves behind debug.
...
- Waiting out a non-cooperative backend after the five-minute heartbeat timeout. The global heartbeat slot still releases at five minutes; late model output never reaches memory or chat. When the provider exits, Debug mode may enrich the already-terminal timeout trace with the late result, including an exact pass, without changing its outcome.
...
2. **`StoredChatMessage` remains the visible transcript.** Turns *point at* message UUIDs (`userMessageID`, `assistantMessageID`). Greeting, fake developer messages, and TTS playback are unchanged. Direct, group, and heartbeat passes have a turn and no assistant message. A heartbeat pass also keeps its compact `HeartbeatRun`; only chat delivery is omitted.
...
6. When the cancelled task later returns, `executionTasks[id]?.token` does not match. In Debug mode, refresh only the exact timed-out turn's tool rows, debug payload, and recovered token counts. Apply the same refresh when the late result is exact PASS; retain the existing generation graph and timeout outcome. Never create a second turn, append memory, post to chat, or change the timeout outcome/scheduling.

Tools that finished before t=5:00 are present immediately. In Debug mode, tools that complete after the timeout snapshot appear when a cancellation-resistant provider finally exits and refreshes the turn, including when its result is exact PASS. `executeHeartbeat` still attaches the recorder snapshot to `HeartbeatModelFailure` so **abort** (token still valid), **model errors**, and late timeout refreshes flush tools.
...
| Exact pass after destination | `targetChat.id` | Yes, `.passed`, tools, and Debug payload/collaboration trace when enabled; no assistant bubble |
...
| Heartbeat pass | Compact `HeartbeatRun` + linked `GenerationTurn`; `ToolInvocation` rows always; `GenerationDebugPayload` and full collaboration trace when Debug is on | Visible in history; no assistant bubble |
...
- **Changes:** Implement the Heartbeat single-writer protocol: preallocated `runID`/`turnID`, scheduler-owned recorder passed into `executeHeartbeat` (checklist: no second `ToolCallRecorder()` in the VM), `onDestinationChat` on `ChatStore` only, slot `outcome` for timeout vs abort, and `recordHeartbeatCompletion` inserting a compact run for every outcome plus a generation turn for every outcome after destination resolution, including exact PASS. Every destination-resolved run retains ordered tool rows; Debug-on runs, including PASS, retain the full payload and correlated collaboration trace. PASS still posts no assistant bubble. New `HeartbeatRun.modelInput`/`modelOutput` always empty/nil. Replace `onModelInput` / `runningModelInputs` with `onDebugPrompt(system, conversation)` filling `slot.debugSystemPrompt` / `debugConversationPrompt` when debug is on; timeout payload uses those two fields, not a concatenated blob. Timeout snapshots the recorder at t=5:00; if a cancellation-resistant provider exits later, Debug mode refreshes that exact timed-out turn's tools and payload without changing its outcome, including when the late result is PASS. Heartbeats Completed expand: Tools list; hide empty Model input/output; show payload when present; distinguish legacy PASS-without-trace, debug-on/no-payload, and debug-off. `RunningHeartbeat` gains `chatID` and `debugCaptureEnabled`.
